...
export default function UpsertCellGroup() {
  ...
  const { id: ministryId, cellId } = useLocalSearchParams<{
    id: string;
    cellId?: string;
  }>();
  ...
  const [members, setMembers] = useState<
    { id: string; name: string; role: "LEADER" | "AUX" | "MEMBER" }[]
  >([]);
    ...
    <View style={styles.container}>
      ...
      >
        <Text style={styles.title}>
          {isEditing ? "Editar Célula" : "Nova Célula"}
        </Text>
              ...
              >
                {item.photo ? (
                  <Avatar.Image
                    ...
                    style={styles.avatar}
                    source={{
                      uri: `https://ichurch-storage.s3.us-east-1.amazonaws.com/${item.photo}`,
                    }}
                  />
                ) : (
                  <Avatar.Icon
